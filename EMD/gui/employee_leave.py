...
class EmployeeLeave:
    def __init__(self, root):
        self.root = root
        self.root.title("Employee Leave")
    ...
    def create_gui(self):
        ...
        # Date (manual entry)
        tk.Label(form_frame, text="DATE (dd/mm/yyyy):", font=("Helvetica", 12, "bold"), fg="white", bg="#CDAA7D").grid(row=3, column=0, padx=10, pady=10, sticky="e")
        tk.Entry(form_frame, textvariable=self.date_var, width=15, font=("Helvetica", 12)).grid(row=3, column=1, padx=10, pady=10, sticky="w")

        # Reason
        tk.Label(form_frame, text="REASON:", font=("Helvetica", 12, "bold"), fg="white", bg="#CDAA7D").grid(row=4, column=0, padx=10, pady=10, sticky="e")
        tk.Entry(form_frame, textvariable=self.reason_var, width=40, font=("Helvetica", 12)).grid(row=4, column=1, padx=10, pady=10, sticky="w")
    ...
    def search_employee(self):
        ...
        try:
            result = self.db.get_employee_details_by_id_no(id_no)  # Use the new method
            if result:
    ...
    def save_data(self):
        ...
        try:
            if not self.db.check_duplicate("id_no", self.id_no_var.get()):
                ...
                return
            leave_date = self.convert_date_format(self.date_var.get())
            # Fetch registration_no based on id_no
            ...
            self.db.insert_employee_leave(
                registration_no=registration_no,
                leave_date=leave_date,
                reason=self.reason_var.get()
            )
            messagebox.showinfo("Success", "Leave data saved successfully!")
            self.reset_form()
        except Exception as e:
